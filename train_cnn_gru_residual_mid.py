...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_mse < best:
        best = val_mse
        torch.save(model.state_dict(), "cnn_gru_residual_lag_mid.pt")

print("Saved → cnn_gru_residual_lag_mid.pt")
...
os.makedirs("eval_results1", exist_ok=True)
results = []
for i in range(len(ds)):
    ...
    plt.savefig(os.path.join("eval_results1", f"{fname.replace('.npz','')}.png"), dpi=150)
    print("Saved attention_prediction_cnn_gru_residual_lag0%d_100.png"%i)
    plt.close()

    # --- Metrics ---
    yt_t = torch.tensor(Y0).unsqueeze(0)
    yp_t = torch.tensor(pred).unsqueeze(0)
    mse_val = ((yt_t - yp_t)**2).mean().item()
    rho_val, _ = spearmanr(Y0, pred)
...
df.to_csv("evaluation_summary1.csv", index=False)
print("\nAll visualizations saved to eval_results1/")
